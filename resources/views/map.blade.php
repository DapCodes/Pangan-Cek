    ...
    <style>
       ...
       ========================================================= */
        ...
        .select2-container--bootstrap-5 .select2-selection__arrow b {
            border-color: var(--control-text) transparent transparent transparent !important;
        }
       ...
       ========================================================= */
        ...
        .location-info {
            background: linear-gradient(180deg, rgba(46, 213, 115, .15), rgba(46, 213, 115, .08));
            border: 1px dashed rgba(46, 213, 115, .35);
            padding: .7rem .9rem;
            border-radius: 10px;
            font-size: .92rem;
            color: #eafff2;
       ...
       ========================================================= */
        ...
        .alert-warning {
            background: linear-gradient(180deg, #ffe08a, #ffc048);
            box-shadow: 0 10px 20px rgba(255, 192, 72, .25);
        }

        .alert-info {
            background: linear-gradient(180deg, #a0c4ff, #5f7cff);
            box-shadow: 0 10px 20px rgba(95, 124, 255, .25);
        }
        ...
        .leaflet-container .leaflet-pane img:focus {
...
<body>
                ...
                <div class="card position-relative">
                    <div class="card-body">
                        <div id="mapInfo" class="map-info">
                            ...
                            <strong>Mode: Laporan Harga</strong> — Klik ganda pada peta untuk menandai lokasi. Data
                            administratif akan terisi otomatis.
                            <div id="loadingGeocode" class="geo-spinner"><i class='bx bx-loader bx-spin'></i> Mengambil
                                data lokasi…</div>
                        </div>
                        <div id="map"></div>
                        ...
                        <div id="mapLegend" class="legend" style="display:none;">
                            <div class="legend-item"><span class="legend-color"
                                    style="background:#2ECC71"></span><span><strong>Aman</strong> — Tidak ada
                                    kelangkaan</span></div>
                            <div class="legend-item"><span class="legend-color"
                    ...
                    <div class="card-body">
                        ...
                        <form id="priceForm" class="row g-3">
                            <div class="col-12">
                                ...
                                <select id="price_commodity_id" name="commodity_id" class="form-select select2"
                                    data-placeholder="Pilih komoditas…" required>
                                    <option value=""></option>
                                    @php
                            ...
                            <div class="col-12">
                                ...
                                <select id="price_province_select" name="province_id" class="form-select select2"
                                    data-placeholder="Otomatis dari peta">
                                    <option value=""></option>
                                </select>
                            ...
                            <div class="col-6">
                                ...
                                <input id="price_lat" name="lat" type="number" class="form-control"
                                    step="0.0000001" readonly required />
                            </div>
                            <div class="col-6">
                                <label class="form-label"><i class='bx bx-current-location'></i> Longitude</label>
                                <input id="price_lng" name="lng" type="number" class="form-control"
                                    step="0.0000001" readonly required />
                            </div>
                    ...
                    <div class="card-body">
                        ...
                        <form id="dearthForm" class="row g-3">
                            <div class="col-12">
                                ...
                                <select id="dearth_commodity_id" name="commodity_id" class="form-select select2"
                                    data-placeholder="Pilih komoditas…" required>
                                    <option value=""></option>
                                    @foreach ($grouped as $cat => $items)
                                        <optgroup label="{{ $cat ?? 'Komoditas' }}">
                            ...
                            <div class="col-12">
                                <label class="form-label"><i class='bx bx-bar-chart-alt'></i> Tingkat
                                    Kelangkaan</label>
                                <select id="severity" name="severity" class="form-select select2"
                                    data-placeholder="Pilih tingkat…" required>
                                    <option value=""></option>
                            ...
                            <div class="col-6">
                                <label class="form-label"><i class='bx bx-current-location'></i> Latitude</label>
                                <input id="dearth_lat" name="lat" type="number" class="form-control"
                                    step="0.0000001" readonly required />
                            </div>
                            <div class="col-6">
                                <label class="form-label"><i class='bx bx-current-location'></i> Longitude</label>
                                <input id="dearth_lng" name="lng" type="number" class="form-control"
                                    step="0.0000001" readonly required />
                            </div>
    ...
    <script>
        ...
        $(function() {
            ...
            Chart.defaults.color = getComputedStyle(document.documentElement).getPropertyValue('--chart-axis')
                .trim();
            ...
            kabupatenLayer = L.geoJSON(kabJabar, {
                style: (feature) => {
                    ...
                    return {
                        ...
                        weight: .8,
                        fillColor: color,
                        fillOpacity: (currentMode === 'dearth' && stats) ? .55 : .18
                    };
                },
                onEachFeature: (feature, layer) => {
                    ...
                    const code = getKabupatenCode(feature);
                    const stats = code ? dearthStatsByRegencyId[code] : null;
                    layer.bindTooltip(tooltipHtml(kabName, stats, getSelectedCommodityName()), {
                        sticky: true
                    });
                    layer.on({
                        ...
                        click: () => {
                            ...
                            L.popup({
                                    maxWidth: 340
                                }).setLatLng(layer.getBounds().getCenter()).setContent(html)
                                .openOn(map);
                        }
                    });
                }
        ...
        async function refreshDearthChoropleth() {
            ...
            try {
                const res = await fetch(`${API_BASE_URL}/dearth/map?days=30&commodity_id=${commodityId}`, {
                    cache: 'no-cache'
                });
                ...
                if (json?.success) {
                    ...
                    if (kabupatenLayer) {
                        kabupatenLayer.eachLayer(layer => {
                            ...
                            layer.setStyle({
                                fillColor: color,
                                fillOpacity: (currentMode === 'dearth' && s) ? .55 : .18
                            });
                            layer.bindTooltip(tooltipHtml(getKabupatenName(layer.feature), s,
                                getSelectedCommodityName()), {
                                sticky: true
                            });
                        });
                    }
        ...
        function handleMapDoubleClick(e) {
            ...
            marker.on('dragend', (ev) => {
                ...
                const newLat = p.lat.toFixed(6),
                    newLng = p.lng.toFixed(6);
        ...
        async function getAdministrativeData(lat, lng) {
            ...
            const prefix = currentMode === 'price' ? 'price' : 'dearth';
            try {
                const res = await $.get(`${API_BASE_URL}/reverse-geocode`, {
                    lat,
                    lng
                });
        ...
        function handlePriceFormSubmit(e) {
            ...
            $.ajax({
                ...
                .fail(xhr => {
                    showAlert('priceAlertContainer', 'danger', xhr.responseJSON?.message || 'Gagal mengirim laporan.');
                })
        ...
        function handleDearthFormSubmit(e) {
            ...
            $.ajax({
                ...
                .fail(xhr => {
                    showAlert('dearthAlertContainer', 'danger', xhr.responseJSON?.message || 'Gagal mengirim laporan.');
                })
        ...
        function switchTab(mode) {
            currentMode = mode;
            $('#priceTab').toggleClass('btn-light', mode === 'price').toggleClass('btn-outline-light text-white', mode !==
                'price');
            $('#dearthTab').toggleClass('btn-light', mode === 'dearth').toggleClass('btn-outline-light text-white', mode !==
                'dearth');
        ...
        function normalizeTrendResponse(res, mode) {
            ...
            if (res.chart && Array.isArray(res.chart.labels) && Array.isArray(res.chart.datasets)) {
                return {
                    labels: res.chart.labels.map(formatDateLabel),
                    datasets: res.chart.datasets
                };
            }
            ...
            if (mode === 'price') {
                ...
                return {
                    ...
                    datasets: [{
                            label: 'Kritis',
                            data: arr.map(x => parseInt(x.critical_count) || 0),
                            fill: false,
                            borderWidth: 2,
                            tension: .3,
                            pointRadius: 2,
                        ...
                        {
                            label: 'Rawan',
                            data: arr.map(x => parseInt(x.high_count) || 0),
                            fill: false,
                            borderWidth: 2,
                            tension: .3,
                            pointRadius: 2,
                        ...
                        {
                            label: 'Waspada',
                            data: arr.map(x => parseInt(x.medium_count) || 0),
                            fill: false,
                            borderWidth: 2,
                            tension: .3,
                            pointRadius: 2,
                        ...
                        {
                            label: 'Rendah',
                            data: arr.map(x => parseInt(x.low_count) || 0),
                            fill: false,
                            borderWidth: 2,
                            tension: .3,
                            pointRadius: 2,
        ...
        function renderTrendChartLine(chartData) {
            ...
            const commonOpts = {
                ...
                plugins: {
                    ...
                    title: {
                        display: true,
                        text: currentMode === 'price' ? 'Tren Harga Komoditas — 30 Hari Terakhir' :
                            'Tren Kelangkaan — 30 Hari Terakhir',
                        color: title,
                        font: {
                            size: 15,
                            weight: 'bold'
                    ...
                    tooltip: currentMode === 'price' ? {
                        callbacks: {
                            label: (c) => `${c.dataset.label}: Rp ${Number(c.parsed.y).toLocaleString('id-ID')}`
                        }
                    } : {}
                },
                scales: {
                    ...
                    y: {
                        ...
                        title: {
                            display: true,
                            text: currentMode === 'price' ? 'Harga (Rp)' : 'Jumlah Laporan',
                            color: axis
                        }
                    }
                }
