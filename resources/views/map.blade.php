...
<head>
  ...
  <style>
    ...
    :root {
      ...
      /* Warna khusus grafik (kontras tinggi) */
      --chart-avg: #ffd166;   /* kuning hangat */
      --chart-min: #00e6a7;   /* hijau toska */
      --chart-max: #ff5b6e;   /* merah cerah */
    ...
    .navbar {
      background:
        linear-gradient(135deg, rgba(255, 122, 89, .95) 0%, rgba(255, 184, 108, .95) 50%, rgba(95, 124, 255, .95) 100%);
      box-shadow:
        0 10px 30px rgba(255, 122, 89, .25),
    ...
    .navbar .nav-pills .btn {
      border-radius: 999px;
      padding: .55rem 1rem;
      box-shadow: 0 10px 20px rgba(0, 0, 0, .08);
      transition: transform .15s ease, box-shadow .2s ease, opacity .2s ease;
    }
    .navbar .nav-pills .btn:hover {
    ...
    .card-header {
      ...
      padding: .9rem 1.1rem; color: #f8fafc;
    }
    .card-body { padding: 1rem 1.1rem 1.2rem; }

    /* =========================================================
       MAP + INFO (dipertahankan)
       ========================================================= */
    ...
    .map-info {
      ...
      border: 1px dashed rgba(95, 124, 255, .35);
      padding: .8rem 1rem; border-radius: 10px; margin-bottom: .75rem; font-size: .95rem; color: #dbeafe;
    }
    ...
    .select2-container--bootstrap-5 .select2-selection { min-height: 44px; border-radius: 12px !important; border: 1px solid var(--border) !important; background: rgba(255, 255, 255, .02) !important; color: #e5e7eb; }
    .select2-container--bootstrap-5 .select2-selection__rendered { color: #e5e7eb !important; }
    .form-control, .form-select { border: 1px solid var(--border); border-radius: 12px; padding: .65rem .9rem; background: rgba(255, 255, 255, .02); color: #e5e7eb; }
    .form-control:focus, .form-select:focus, .select2-container--bootstrap-5 .select2-selection:focus {
      border-color: transparent; box-shadow: 0 0 0 .2rem var(--ring), 0 0 0 .35rem var(--ring-2); outline: none;
    }
    ::placeholder { color: #94a3b8 !important; }

    .btn-primary {
      background: radial-gradient(120% 180% at 10% 10%, rgba(255, 255, 255, .25), transparent 40%),
                  linear-gradient(135deg, var(--primary) 0%, var(--primary-2) 55%, var(--secondary) 100%);
      border: none; border-radius: 12px; padding: .8rem 1.2rem; font-weight: 700;
      box-shadow: 0 14px 30px rgba(255, 122, 89, .25), 0 6px 16px rgba(95, 124, 255, .18);
      color: #0b1220; text-shadow: 0 1px 0 rgba(255, 255, 255, .35);
      transition: transform .12s ease, box-shadow .18s ease, filter .18s ease;
    }
    .btn-primary:hover { transform: translateY(-1px); filter: saturate(1.05); box-shadow: 0 18px 36px rgba(255, 122, 89, .35), 0 10px 24px rgba(95, 124, 255, .25); }

    /* =========================================================
       LEGEND & INFO (dipertahankan)
       ========================================================= */
    ...
    /* =========================================================
       CHART – Gaya Baru (anti tabrakan UI)
       ========================================================= */
    .chart-container {
      height: 360px;
      /* Ganti jadi panel solid agar grafik kontras & tidak “tenggelam” */
    ...
    /* Pastikan canvas transparan (biar pakai background container saja) */
...
<body>
  ...
  <nav class="navbar navbar-expand-lg">
    <div class="container-xxl position-relative">
      ...
      <!-- NOTE: ribbon "Realtime Crowdsource" DIHAPUS sesuai permintaan -->
            ...
            <form id="priceForm" class="row g-3">
              ...
              <div class="col-12">
                ...
                <div class="input-group">
                  <span class="input-group-text bg-transparent text-white border-0 border-bottom">Rp</span>
...
<script>
  ...
  function scoreToColor(avg) {
    if (avg === 0) return '#2ECC71';   // aman
    if (avg <= 1) return '#F1C40F';    // waspada
    if (avg <= 2) return '#E67E22';    // rawan
    return '#E74C3C';                  // kritis
  }
  ...
  function initializeMap() {
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        () => { /* diamkan jika user menolak geolokasi */ }
      );
    }
  ...
  async function loadGeoLayers() {
    ...
    provinceLayer = L.geoJSON(provJabar, {
      style: { color: '#7aa2ff', weight: 1.2, fillOpacity: 0.05 },
    ...
    kabupatenLayer = L.geoJSON(kabJabar, {
      style: (feature) => {
        ...
        return { color: '#9aa5b1', weight: 0.8, fillColor: color, fillOpacity: (currentMode === 'dearth' && stats) ? 0.55 : 0.18 };
      },
      ...
      if (json?.success) {
        ...
        if (kabupatenLayer) {
          kabupatenLayer.eachLayer(layer => {
            ...
            layer.setStyle({ fillColor: color, fillOpacity: (currentMode === 'dearth' && s) ? 0.55 : 0.18 });
  ...
  function loadRegencies(prefix, provinceId, selectedId = null) {
    ...
    return $.get(`/regencies/${provinceId}`).then(res => {
      ...
      res.data.forEach(i => opt += `<option value="${i.id}">${i.name}</option>`);
      $reg.html(opt); if (selectedId) $reg.val(String(selectedId)); $reg.trigger('change.select2');
    });
  }
  function loadDistricts(prefix, regencyId, selectedId = null) {
    ...
    return $.get(`/districts/${regencyId}`).then(res => {
      ...
      res.data.forEach(i => opt += `<option value="${i.id}">${i.name}</option>`);
      $dist.html(opt); if (selectedId) $dist.val(String(selectedId)); $dist.trigger('change.select2');
    });
  }
  function loadVillages(prefix, districtId, selectedId = null) {
    const $vill = $(`#${prefix}_village_select`);
    if (!districtId) { $vill.html('<option value="">-- Pilih Desa/Kelurahan --</option>').trigger('change.select2'); return Promise.resolve(); }
    return $.get(`/villages/${districtId}`).then(res => {
      let opt = '<option value="">-- Pilih Desa/Kelurahan --</option>';
      res.data.forEach(i => opt += `<option value="${i.id}">${i.name}</option>`);
      $vill.html(opt); if (selectedId) $vill.val(String(selectedId)); $vill.trigger('change.select2');
    });
  }
  ...
  async function loadTrendChart() {
    ...
    // Coba beberapa endpoint supaya kompatibel
    const urls = [
      `${API_BASE_URL}/trend?${qs}`,                 // backend lama: satu endpoint
      `${API_BASE_URL}/${currentMode}/trend?${qs}`   // backend baru: per-mode
    ];
  ...
  // Normalisasi bentuk respons → {labels, datasets}
  function normalizeTrendResponse(res, mode) {
    if (!res) return null;

    // Jika backend sudah mengirim format siap Chart.js
    ...
    // Bentuk lama: data/raw berupa array per tanggal
    ...
    if (mode === 'price') {
      ...
      // dearth → LINE CHART untuk tiap severity
      return {
        labels,
        datasets: [
          ...
          // { label: 'Total Laporan', data: arr.map(x => parseInt(x.total_reports) || 0), fill:false, borderWidth:2, borderDash:[6,6], tension:.3, pointRadius:2, pointHoverRadius:4 },
        ]
      };
    }
  }
  ...
  function renderTrendChartLine(chartData) {
    ...
    // Warna dari CSS variable (fallback jika kosong)
    ...
    // Set warna default jika dataset belum punya borderColor
